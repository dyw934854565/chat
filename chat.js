...
function renderDishes(dishes) {
  return (0, _from2.default)({ length: Math.ceil(dishes.length / 2) }).map(function (val, index) {
    var items = [dishes[index * 2], dishes[index * 2 + 1]];
...
new Chat({
  ...
  afterRender: function afterRender() {
    ...
    $.post({
      ...
      success: function success(result) {
        _this.run('onRecvMsg', result.response);
      },
      dataType: 'json',
  ...
  send: function send(msg) {
    ...
    return $.post({
      ...
      data: (0, _stringify2.default)({ 'sentence': msg }),
      success: function success(result) {
        if (typeof result.response == "string") {
          _this2.run('onRecvMsg', result.response);
        } else {
          var order = result.response.order;
          _this2.run('onRecvMsg', "已为您生成菜单");
          $("#order_detail").html(renderMenu(result.response));
          $('.top-item').show();
          addChart('order_material', '食材', (0, _entries2.default)(order.materialDistribution).map(function (_ref) {
            var name = _ref[0],
                value = _ref[1];
            return { name: name, value: value };
          }));
          addChart('order_taste', '味道', (0, _entries2.default)(order.tasteDistribution).map(function (_ref2) {
            var name = _ref2[0],
                value = _ref2[1];
            return { name: name, value: value };
          }));
          _this2.run('updateBodyHeight');
          _this2.run('scrollToBottom');
        }
      },
      dataType: 'json',
      contentType: 'application/json'
  ...
  updateBodyHeight: function updateBodyHeight() {
...
var $entries = __webpack_require__(35)(true);
